...
- **👁 Agent capability hint** — text-only models almost never figure out on their own that reading an image file is how they "see." On handoff-target turns the extension appends a short system-prompt note explaining that pastes arrive as descriptions and any image file can be inspected via `read` (disable with `agentHint: false`).
...
| `agentHint` | `true` | **Teach the agent how it sees.** When on, handoff-target turns get a short `## Image perception (vision handoff)` section appended to the system prompt: pasted/attached images arrive as text descriptions, and any image file can be inspected by calling the read tool on its path (the description is swapped in automatically). Without this, text-only models rarely discover the read-an-image path on their own — the extension works but sits unused. Set `false` to keep the system prompt untouched (e.g. if your AGENTS.md already documents the capability). |
